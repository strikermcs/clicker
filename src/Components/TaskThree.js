...
const TaskTwo = ({ showModal3, setShowModal3 }) => {
  const {id, balance, setBalance, taskCompleted3, setTaskCompleted3} = useUser();
  ...
  const taskID = "task_3003"; // Assign a unique ID to this task
  ...
  const handleTaskLinkClick = () => {
    window.open("https://t.me/scarl900");
  ...
  const handleVerify = async () => {
    ...
    const response = await fetch(
      `https://api.telegram.org/bot${BOT_TOKEN}/getChatMember?chat_id=-1002227548826&user_id=${id}`
    );
    ...
    if (data.ok && (data.result.status === "member" || data.result.status === "administrator" || data.result.status === "creator")) {
      ...
      setTimeout(() => {
        setMessage(
          "Please join the Telegram community first before you can claim this task bonus."
        );
      }, 1000);
      setCounter(15);
        ...
        <div className="fixed z-50 left-0 right-0 top-0 bottom-0 flex justify-center taskbg px-[16px] h-full">
          <div className={`w-full flex flex-col items-center justify-start`}>
            ...
            <div className="flex w-full flex-col">
              <h1 className="text-[20px] font-semibold">Subscribe To Our Coding Community</h1>
              <p className="text-[#9a96a6] text-[16px] font-medium pt-1 pb-10">
              Join our coding community to get web3 development updates and ideas for your own projects!
              </p>
