...
export const Address = () => {
  const {id, walletAddress, isAddressSaved, setWalletAddress, setIsAddressSaved} = useUser()
...
useEffect(() => {
 ...
 {walletAddress !== '' ? (
 <button
      onClick={() => setOpenInfo(true)} className={`bg-cards flex h-full w-fit rounded-full items-center py-[10px] px-3 relative space-x-1`}>

<img src='/wallet.webp' alt="connect"
className="w-[16px] -mt-[2px]"/>

<div className="text-[13px] small-text2 pr-3 text-left text-nowrap text-white flex flex-1 flex-col">
<h4 className='font-semibold text-[#d1d1d1] line-clamp-1 break-all text-wrap'>{walletAddress}</h4>
</div>
<IoCheckmarkCircle size={20} className='text-[#d1d1d1]' />
</button>
 ): (
    <button
...
<div className="w-fit flex justify-center absolute right-6 top-6">
          ...
          className="w-full"/>
              ...
              <h3 className='font-semibold text-green text-[22px] w-full text-center'>Your wallet is connected</h3>
              <p className="pb-6 text-[#bfbfbf] text-[14px] w-full text-center">
...
<button onClick={disco} className='bg-red w-fit py-2 px-10 text-center rounded-[25px]'>
    Disconnect wallet
</button>
