...
const AdminSettings = () => {
  ...
  const fetchSettings = async () => {
    const docRef = doc(db, "settings", "61gP4nP9aXZCx5D1ywgZ"); // Replace with your actual document ID
    ...
    <div id='refer' className="w-full flex flex-col space-y-4 h-[100vh] scroller pt-4 overflow-y-auto pb-[150px]">
      ...
      <div className="flex w-full flex-wrap gap-3">
        ...
        <div className='flex flex-col w-full sm:w-[49%] gap-1'>
          <label className='text-[13px] pl-1 pb-[2px] font-medium'>Show Wallet Connect</label>
          <select
            name="showWalletConnect"
            value={settings.showWalletConnect}
        ...
        <button onClick={handleUpdateSettings} className="bg-green font-semibold text-[15px] rounded-[6px] w-[50%] sm:w-[200px] h-fit px-4 py-3 text-[#fff]">Update Settings</button>
      </div>
